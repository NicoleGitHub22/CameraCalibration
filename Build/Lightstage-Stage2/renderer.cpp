//=============================================================================
// Reads sh response files. Renders the final image from acquired surface
// information and stores all results as image files. Passes the results to the
// GUI.
//=============================================================================
#include "renderer.h"
#include <QDebug>
...
Renderer::Renderer(QObject *parent) : QObject(parent)
{
    ...
    // lights = {Light(Vector3D(0, 1, 0.5), 10, QColor(255, 255, 255))};
    ...
    for(int u = renderArea.xStart; u < renderArea.xEnd; u++){
        for(int v = renderArea.yStart; v < renderArea.yEnd; v++){
            ...
            vector<double> normal = {   (double)normals.pixelColor(u, v).red() / 255,
                                        (double)normals.pixelColor(u, v).blue() / 255, // Previously green
                                        (double)normals.pixelColor(u, v).green() / 255 // Previously blue
                                    };
            ...
            for(Light &light : lights){
                ...
                //is the light behind the surface? if yes, skip this lightsource
                if(theta_i <= 0){
                     color.setRgb(0,0,0);
                     continue;
                }
                ...
                // vector<double> camera_dir = {-1, -1, -0.5};
...
void Renderer::onRender()
{
    ...
    QString filepath = "/home/bretznic/Documents/GitHub/CameraCalibration/Lightstage/Build/Server/";
